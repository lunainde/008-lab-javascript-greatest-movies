...
function orderByYear(moviesArray) {
  const moviesCopy = [...moviesArray].sort((a, b) => {
    if (a.year !== b.year) {
...
function turnHoursToMinutes(moviesArray) {
  const movieInMin = moviesArray.map((oldMovie) => {
    const movie = { ...oldMovie };

    let minutes = 0;

    // Change Hours to Minutes, add to the variable
    if (movie.duration.includes("h")) {
      const strHours = movie.duration.split(" ")[0].replace("h", "");
      minutes += parseInt(strHours) * 60;
    }

    // add to the variable
    if (movie.duration.includes("min")) {
      const strMinutes = movie.duration.split(" ")[1].replace("min", "");
